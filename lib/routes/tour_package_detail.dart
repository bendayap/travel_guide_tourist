...
class _TourPackageDetailState extends State<TourPackageDetail> {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Package Title: ${data['packageTitle']}'),
            // const SizedBox(height: 4.0),
            // const Text('Package Type: '),
            // for(int i=0; i<data['packageType'].length; i++)
            //   Text('\n    ${data['packageType'][i]}'),
            const SizedBox(height: 4.0),
            Text('Tour Guide: ${data['ownerId']}'),
            const SizedBox(height: 4.0),
            Text('Duration: ${data['duration']} day(s)'),
            const SizedBox(height: 4.0),
            ...
            ElevatedButton(
              onPressed: () async {
                if (user != null) {
                  ...
                  if (eWalletData['balance'] < data['price']) {
                    await showDialog(
                      context: context,
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        content: SingleChildScrollView(
                          child: ListBody(
                            children: <Widget>[
                              const Text('Your wallet does not have sufficient balance for this tour package.'),
                              Text('\nPackage Price: ${data['price']}'),
                              Text('Your Balance: ${eWalletData['balance']}'),
                            ],
                          ),
                        ),
                        actions: <Widget>[
                    ...
                    Navigator.pushNamed(context, '/request_booking',
                        arguments: {
              ...
              onPressed: () async {
                if (user != null) {
                  ...
                  if (!chatroomExists) {
                    ...
                    updateChatroomTitle(chatroomId, data['packageTitle']);
                  }
                  await Navigator.push(
                    context,
                    MaterialPageRoute(
  ...
  void updateChatroomTitle(String chatroomId, String packageTitle) {
    final docChatroom =
    FirebaseFirestore.instance.collection('chatrooms').doc(chatroomId);
    docChatroom.update({
      'chatroomTitle': packageTitle,
    });
